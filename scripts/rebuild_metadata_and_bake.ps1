...
python recreate_metadata.py

# Paths
$fullSource = "metadata.json"
...
if (-Not (Test-Path $fullSource)) {
    Write-Error "ERROR: No rebuilt metadata.json found at $fullSource"
    exit 1
}
...
if ($fileSizeKB -lt 5) {
    Write-Error "ERROR: metadata.json is too small (<5 KB) - rebuild may have failed."
    exit 1
}

# 4. Copy to backend baked metadata
Copy-Item $fullSource $backendTarget -Force
Write-Host "SUCCESS: Copied metadata.json to $backendTarget"

# 5. Validate backend JSON
try {
    $json = Get-Content $backendTarget -Raw | ConvertFrom-Json
    Write-Host "SUCCESS: base_metadata.json is valid JSON."
} catch {
    Write-Error "ERROR: base_metadata.json is not valid JSON."
    exit 1
}
...
try {
    ...
    Write-Host "SUCCESS: Created lightweight UI metadata at $frontendTarget"
} catch {
    Write-Error "ERROR: Failed to generate UI metadata: $_"
    exit 1
}
