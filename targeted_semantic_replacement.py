...
def targeted_semantic_replacement():
    """
    Targeted replacement of ONLY the semantic detection function's reference queries:
    1. Find the detect_domain_semantic function specifically
    2. Replace only its domain_references section
    3. Leave other legitimate sections intact
    """
    
    print("🎯 TARGETED SEMANTIC REFERENCE QUERY REPLACEMENT")
    print("=" * 60)
...
"""
    ...
    print(f"\n🔍 Searching for detect_domain_semantic function...")
    
    # Find the detect_domain_semantic function and its domain_references section
    # Look for the pattern: def detect_domain_semantic(...) ... domain_references = { ... }
    
    # First, find the function start
    function_pattern = r'def detect_domain_semantic\(query: str\) -> dict:.*?domain_references = \{'
    function_match = re.search(function_pattern, content, re.DOTALL)
    
    if not function_match:
        print("❌ Could not find detect_domain_semantic function with domain_references")
        return
    
    function_start = function_match.start()
    domain_ref_start = function_match.end() - len('domain_references = {')
    
    print(f"✅ Found detect_domain_semantic function starting at line {content[:function_start].count(chr(10)) + 1}")
    
    # Now find the matching closing brace for domain_references
    brace_count = 1
    pos = domain_ref_start + len('domain_references = {')
    ...
    print(f"✅ Found domain_references section (lines {content[:domain_ref_start].count(chr(10)) + 1}-{content[:end_pos].count(chr(10)) + 1})")
    
    # Extract the current content for verification
    current_section = content[domain_ref_start:end_pos]
    ...
    before_section = content[:domain_ref_start]
    ...
    # Check if legacy content was removed from the semantic function
    legacy_removed = True
    for indicator in legacy_indicators:
    ...
    if legacy_removed:
        print("✅ Legacy content successfully removed from semantic function")
    
    print("\n" + "=" * 60)
    print("🎯 TARGETED REPLACEMENT COMPLETE")
