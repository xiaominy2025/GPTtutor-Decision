#!/usr/bin/env python3
"""
Pre-cleanup test corpus to capture golden outputs before legacy code removal.
"""
...
def capture_golden_outputs():
    """Capture JSON outputs for all test queries before cleanup."""
    golden_outputs = {}
    
    for i, query in enumerate(TEST_QUERIES, 1):
        try:
            print(f"Capturing Q{i}: {query[:50]}...")
            result = query_engine.run_query_once(query)
            golden_outputs[f"Q{i}"] = {
                "query": query,
                "output": result
            }
    ...
    # Save to file
    with open('golden_outputs_pre_cleanup.json', 'w', encoding='utf-8') as f:
        json.dump(golden_outputs, f, indent=2, ensure_ascii=False)
    
    print(f"Captured {len(golden_outputs)} test outputs to golden_outputs_pre_cleanup.json")
