...
LAMBDA-READY VERSION: Original logic preserved, performance optimizations only
"""
...
# === LAMBDA PERFORMANCE OPTIMIZATIONS (NO LOGIC CHANGES) ===
import torch
...
# === LAZY SINGLETON INITIALIZATION (NO LOGIC CHANGES) ===
...
def get_embeddings_model():
    """Lazy singleton for embeddings model - ORIGINAL MODEL PRESERVED"""
    global _embeddings_model
    if _embeddings_model is None:
        # KEEP ORIGINAL MODEL: all-mpnet-base-v2 (not the smaller one)
        _embeddings_model = SentenceTransformer('all-mpnet-base-v2', device='cpu')
        print("✅ Embeddings model loaded (CPU-only, original model preserved)")
...
def get_faiss_index():
    ...
    if _faiss_index is None:
        ...
        if index_path.exists():
            ...
            _faiss_index = faiss.IndexFlatIP(768)  # Original dimension for all-mpnet-base-v2
    return _faiss_index
...
def get_openai_client():
    ...
    if _openai_client is None:
        import httpx
        proxy_settings = {}
        if os.getenv('HTTP_PROXY'): proxy_settings['http'] = os.getenv('HTTP_PROXY')
        if os.getenv('HTTPS_PROXY'): proxy_settings['https'] = os.getenv('HTTPS_PROXY')
        if os.getenv('NO_PROXY'): proxy_settings['no_proxy'] = os.getenv('NO_PROXY')
...
# Initialize OpenAI client using lazy loading
