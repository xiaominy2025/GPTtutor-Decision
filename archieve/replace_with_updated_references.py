import json
import os
import re

def replace_with_updated_references():
    """Replace the current placeholder reference queries in query_engine.py with the new 79 queries from reference_queries_updated.json"""
    
    # Read the updated reference queries from current directory (project root)
    json_path = 'reference_queries_updated.json'
    with open(json_path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    
    print(f"Found {len(data)} updated reference queries")
    
    # Organize queries by domain
    domain_mapping = {
        'S': 'strategic',
        'T': 'technical', 
        'H': 'behavioral',
        'N': 'negotiation'
    }
    
    new_domain_references = {}
    for item in data:
        query = item['query']
        domains = item['domains']
        
        # Map domain codes to full names
        for domain_code in domains:
            domain_name = domain_mapping.get(domain_code)
            if domain_name:
                if domain_name not in new_domain_references:
                    new_domain_references[domain_name] = []
                new_domain_references[domain_name].append(query)
    
    print("Organized queries by domain:")
    for domain, queries in new_domain_references.items():
        print(f"  {domain}: {len(queries)} queries")
    
    # Create the new domain_references content
    new_domain_references_content = "        # 79 updated domain-specific reference queries (hardcoded for performance)\n        domain_references = {\n"
    for domain_name, queries in new_domain_references.items():
        new_domain_references_content += f"            '{domain_name}': [\n"
        for query in queries:
            new_domain_references_content += f'                "{query}",\n'
        new_domain_references_content += "            ],\n"
    new_domain_references_content += "        }\n"
    
    # Read the current query_engine.py from Repeatability subfolder
    query_engine_path = 'Repeatability/query_engine.py'
    with open(query_engine_path, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Find and replace the domain_references section
    start_pattern = r"# 79 domain-specific reference queries \(hardcoded for performance\)\s*\n\s*domain_references = \{"
    end_pattern = r"\s*\}\s*\n\s*# Generate embeddings for query and domain references using OpenAI"
    
    start_match = re.search(start_pattern, content)
    end_match = re.search(end_pattern, content)
    
    if start_match and end_match:
        start_pos = start_match.start()
        end_pos = end_match.start()
        
        # Replace the entire domain_references section
        before_section = content[:start_pos]
        after_section = content[end_pos:]
        
        new_content = before_section + new_domain_references_content + after_section
        
        # Write the updated content
        with open(query_engine_path, 'w', encoding='utf-8') as f:
            f.write(new_content)
        
        print(f"✅ Successfully updated {query_engine_path} with {len(data)} new reference queries")
        print("Domain distribution:")
        for domain, queries in new_domain_references.items():
            print(f"  {domain}: {len(queries)} queries")
    else:
        print("❌ Could not find the domain_references section in detect_domain_semantic function")
        print("This might mean the file structure has changed or the section was already updated")

if __name__ == "__main__":
    replace_with_updated_references()
