# ================================
# V1.6.6.6 BULLETPROOF DEPLOYMENT WITH API GATEWAY
# Addresses ALL previous deployment failures + Custom Domain
# ================================
...
param (
    ...
    [string]$DomainName = "engentlab.com",
    [string]$ApiSubdomain = "api",
    [switch]$FreshStart = $false,
    [switch]$SkipTests = $false,
    [switch]$SkipApiGateway = $false
)
...
Write-Host "🚀 === V1.6.6.6 BULLETPROOF DEPLOYMENT WITH API GATEWAY ===" -ForegroundColor Cyan
Write-Host "Target: $LambdaName in $Region" -ForegroundColor Yellow
Write-Host "Account: $AccountId" -ForegroundColor Yellow
Write-Host "Domain: $ApiSubdomain.$DomainName" -ForegroundColor Yellow
...
if (-not $SkipApiGateway) {
    Write-Host "`n🌐 Setting up API Gateway..." -ForegroundColor Green
    
    # Create API Gateway
    Write-Host "📝 Creating REST API..." -ForegroundColor Green
    try {
        $apiResponse = aws apigateway create-rest-api `
            --name "engent-labs-v1666-api" `
            --description "API Gateway for V1.6.6.6 Backend" `
            --region $Region
        
        if ($LASTEXITCODE -ne 0) {
            Write-Error "❌ API Gateway creation failed"
            exit 1
        }
        ...
        Write-Error "❌ API Gateway creation failed: $_"
        exit 1
    }
    
    # Get root resource ID
    Write-Host "📝 Getting root resource..." -ForegroundColor Green
    try {
        $resourcesResponse = aws apigateway get-resources --rest-api-id $apiId --region $Region
        $rootResourceId = ($resourcesResponse | ConvertFrom-Json).items[0].id
    ...
    Write-Host "📝 Creating proxy resource..." -ForegroundColor Green
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            Write-Error "❌ Proxy resource creation failed"
            exit 1
        }
        
        $proxyResourceId = ($proxyResponse | ConvertFrom-Json).id
        Write-Host "✅ Proxy resource created: $proxyResourceId" -ForegroundColor Green
    } catch {
        Write-Error "❌ Proxy resource creation failed: $_"
        exit 1
    }
    ...
    try {
        ...
        aws lambda add-permission `
            --function-name $LambdaName `
            --statement-id "AllowApiGatewayInvoke-$timestamp" `
        ...
        Write-Warning "⚠️ Lambda permission may already exist: $_"
    }
    
    # Create integration for proxy resource
    Write-Host "📝 Creating proxy integration..." -ForegroundColor Green
    try {
        ...
        Write-Host "✅ Proxy integration created" -ForegroundColor Green
    } catch {
        Write-Error "❌ Proxy integration failed: $_"
        exit 1
    }
    
    # Create integration for root resource
    Write-Host "📝 Creating root integration..." -ForegroundColor Green
    try {
        aws apigateway put-integration `
            --rest-api-id $apiId `
        ...
        Write-Host "✅ Root integration created" -ForegroundColor Green
    } catch {
        Write-Error "❌ Root integration failed: $_"
        exit 1
    }
    ...
    # Create custom domain (if domain is provided)
    if ($DomainName -and $ApiSubdomain) {
        Write-Host "`n🌐 Setting up custom domain..." -ForegroundColor Green
        
        # Check if ACM certificate exists for the domain
        Write-Host "📝 Checking SSL certificate..." -ForegroundColor Green
        try {
            $certResponse = aws acm list-certificates --region $Region
            $certificates = $certResponse | ConvertFrom-Json
            $domainCert = $certificates.CertificateSummaryList | Where-Object { $_.DomainName -eq "*.$DomainName" -or $_.DomainName -eq $DomainName }
            
            if ($domainCert) {
                $certArn = $domainCert.CertificateArn
                Write-Host "✅ Found certificate: $certArn" -ForegroundColor Green
            } else {
                Write-Warning "⚠️ No SSL certificate found for $DomainName"
                Write-Host "   You'll need to create an SSL certificate in ACM for $DomainName" -ForegroundColor Yellow
                Write-Host "   Or use the API Gateway URL: $apiUrl" -ForegroundColor Yellow
            }
        } catch {
            Write-Warning "⚠️ Could not check SSL certificates: $_"
        }
        
        # Create custom domain name
        if ($certArn) {
            Write-Host "📝 Creating custom domain name..." -ForegroundColor Green
            try {
                aws apigateway create-domain-name `
                    --domain-name "$ApiSubdomain.$DomainName" `
                    --certificate-arn $certArn `
                    --region $Region
                
                Write-Host "✅ Custom domain created: $ApiSubdomain.$DomainName" -ForegroundColor Green
            } catch {
                Write-Warning "⚠️ Custom domain creation failed (may already exist): $_"
            }
        }
    }
    
} else {
    Write-Host "`n⚠️ Skipping API Gateway setup (using Function URL only)" -ForegroundColor Yellow
}

# ================================
# CREATE FUNCTION URL (FALLBACK)
# ================================

Write-Host "`n🌐 Creating function URL (fallback)..." -ForegroundColor Green
try {
    $timestamp = Get-Date -Format "yyyyMMddHHmmss"
...
if (-not $SkipTests) {
    ...
    if ($apiUrl) {
        Write-Host "`n🔍 Testing via API Gateway..." -ForegroundColor Green
        try {
            ...
            Write-Warning "⚠️ API Gateway test failed: $_"
        }
    }
...
if ($functionUrl) {
    Write-Host "`n🌐 Function URL (Fallback): $functionUrl" -ForegroundColor Cyan
...
if ($apiUrl) {
    ...
    Write-Host "Query Endpoint: $apiUrl/query" -ForegroundColor Cyan
}

if ($DomainName -and $ApiSubdomain) {
    Write-Host "`n🌐 Custom Domain (when configured): https://$ApiSubdomain.$DomainName" -ForegroundColor Cyan
    Write-Host "Health Check: https://$ApiSubdomain.$DomainName/health" -ForegroundColor Cyan
    Write-Host "Query Endpoint: https://$ApiSubdomain.$DomainName/query" -ForegroundColor Cyan
}

Write-Host "`n📋 Next Steps:" -ForegroundColor Green
if ($apiUrl) {
    Write-Host "1. Test API Gateway: $apiUrl/health" -ForegroundColor White
    Write-Host "2. Update your frontend to use: $apiUrl" -ForegroundColor White
}
if ($functionUrl) {
    Write-Host "3. Test Function URL: $functionUrl/health" -ForegroundColor White
}
Write-Host "4. Monitor CloudWatch logs for any issues" -ForegroundColor White

if ($DomainName -and $ApiSubdomain) {
    Write-Host "`n🔧 Domain Setup:" -ForegroundColor Green
    Write-Host "1. Create SSL certificate in ACM for $DomainName" -ForegroundColor White
    Write-Host "2. Update DNS to point $ApiSubdomain.$DomainName to API Gateway" -ForegroundColor White
    Write-Host "3. Test custom domain: https://$ApiSubdomain.$DomainName/health" -ForegroundColor White
}
...
Write-Host "`n✅ BULLETPROOF DEPLOYMENT WITH API GATEWAY COMPLETE!" -ForegroundColor Green
