...
def load_data_lazily():
    ...
    if _index is None:
        try:
            # Use course directory from environment or default to current directory
            course_dir = os.environ.get('COURSE_DIR', '/var/task/courses')
            default_course = os.environ.get('DEFAULT_COURSE', 'decision')
            
            # Try to load from course-specific directory first
            faiss_path = os.path.join(course_dir, default_course, "vector_index.faiss")
            metadata_path = os.path.join(course_dir, default_course, "metadata.json")
            
            # Fallback to current directory if course-specific files don't exist
